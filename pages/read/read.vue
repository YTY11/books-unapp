...
<template>
	<view class="book">
		...
		<view class="cover container" :class="{container0: background === 1, container1: background === 2,container2: background === 3}" :style="{zIndex: 201, transform: `translate${cover.pageTranslate[turnType]}`, transition: `transform ${showAnimation?turnPageTime:0}s`,
			...
			<!-- 图片懒加载 -->
			<u-lazy-load class="book-img" :image="bookData.Img" :loading-img="loadingImg" :error-img="errorImg">
			</u-lazy-load>
			<view class="book-name" style="margin-top: 20rpx;">
				{{bookData.Name}}
			</view>
		</view>
		<!-- ************************** -->
		...
		<view class="menu" :style="{height: `100%`, width: `${windowWidth}px`}" v-if="menuShow" @touchend="closeMenu">
			...
			<view class="setting" :style="{bottom: settingShow ? 0 : `-100%`}" @touchend.stop>
				<view class="item">
					...
					<view style="margin-right: 20rpx;">{{fontSize}}</view>
...
<script>
	import battery from '../../components/read/battery.vue'
	import virtualList from '../../components/read/virtualList.vue'
	import {
		traditionalized,
		simplized,
		dateToStr
	...
	export default {
		...
		onLoad(option) {
			...
			if(option.chapter != undefined){
				let chapter = JSON.parse(decodeURIComponent(option.chapter))
				this.directoryChapter = chapter;
			}
			this.getSystemInfo()
			
			let readHistoryLog = uni.getStorageSync('READ_HISTORY_LOG')
			if (typeof readHistoryLog !== 'object') {
				readHistoryLog = []
			}
			readHistoryLog.unshift({
				Name:this.bookData.Name,
				Time:this.$bookUtils.dateFormat('YYYY-MM-DD HH:mm:ss',new Date())
			})
			uni.setStorageSync('READ_HISTORY_LOG',readHistoryLog)
		},
		onUnload() {
		...
		methods: {
			...
			getSystemInfo() {
				...
				this.fontSize = uni.getStorageSync('FONT_SIZE')
				...
				this.lineHeight = uni.getStorageSync('LINE_HEIGHT')
				...
				this.turnType = uni.getStorageSync('TRUN_TYPE')
			...
			async initPage() {
				...
				//是否显示封面
				if (this.history.chapterIndex == 0 && this.history.progress == 0 && this.directoryChapter == 0 ) {
					this.cover.pageTranslate = [
						`(0,0)`,
						`(0,0)`,
						`(0,0)`
				...
				if(this.directoryChapter != 0){
					await this.goToChapter(this.directoryChapter.index)
					this.directoryChapter = 0
				}
				uni.hideLoading()
			...
			async bigSize() {
				...
				this.fontSize += 2
				uni.setStorageSync('FONT_SIZE', this.fontSize)
				this.calcHeight()
			...
			async changeLineHeight(lineHeight) {
				...
				if (lineHeight === this.lineHeight) {
					...
					uni.setStorageSync('LINE_HEIGHT', this.lineHeight)
					this.calcHeight()
					await this.calcCurChapter()
					let page = Math.floor((this.curChapter.totalPage - 1) * progress)
					this.goToPage(page)
			...
			changeTurnType(turnType) {
				if (turnType === this.turnType) {
					...
					uni.setStorageSync('TURN_TYPE', this.turnType)
				}
			},
			...
			getDirectoryList(showLoading) {
				...
				return getBookDirectoryData(this.bookId).then(res => {
					if (showLoading) {
						uni.hideLoading()
					}
					//格式化json字符串
					...
					if (res.info === "success") {
						...
						let directoryData = this.directoryData.list
						// 生成目录
						let index = 0
						directoryData.forEach(data => {
							data.list.forEach(item => {
								this.directoryList.push({
									index: index++,
									chapterId: item.id, //注意：这个chapterId用于获取章节内容而不是index
									name: item.name
								})
							})
						})
			...
			getOneChapter(chapterId, showLoading) {
				...
				return getBookTextData(this.bookId, chapterId).then(res => {
					if (showLoading) {
						uni.hideLoading()
					}
					if (res.info === "success") {
						this.bookTextData = res.data
						//解决v-html 无法解析 "\r\n" 问题
						let text = this.bookTextData.content.replace(/\r\n/g,"<br/>")
							this.tmpChapter.text = `${this.bookTextData.cname}</p>` + text //模拟数据
